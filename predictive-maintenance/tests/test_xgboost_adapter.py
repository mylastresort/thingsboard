...
class TestXGBoostCapabilities:
    def test_capabilities(self):
        config = SupervisedConfig(name="xgboost", algorithm_type="supervised", task_type=TaskType.REGRESSION)
        algo = XGBoostAdapter(config)
        ...
        assert caps.handles_missing_values is True
        assert caps.handles_categorical is True


class TestXGBoostBinaryClassification:
    def test_train(self, binary_config, classification_data):
        X, y = classification_data
        algo = XGBoostAdapter(binary_config)
    ...
    def test_predict(self, binary_config, classification_data):
        ...
        assert output.metadata["algorithm"] == "xgboost"
...
class TestXGBoostMulticlassClassification:
...
class TestXGBoostRegression:
    def test_train_regression(self, regression_config, regression_data):
        X, y = regression_data
        algo = XGBoostAdapter(regression_config)
        metrics = algo.train(X, y)
        assert algo.is_trained is True
