...
- REST endpoints in this gateway are schema-first. Update `api-specs/openapi/` (split schema files) first, run `make bundle-openapi` to reassemble, let OpenAPI Generator produce the `org.tb.quarkus.api.*Api` interface and `org.tb.quarkus.model.*` DTOs, then implement that generated interface in `src/main/java/org/tb/quarkus/controller/*ApiImpl.java`. Do not add standalone JAX-RS resources for new public endpoints; `mp.openapi.scan.disable` is enabled, so scanner-only endpoints will not appear in Swagger UI.
